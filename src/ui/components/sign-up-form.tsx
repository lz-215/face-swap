...
export function SignUpForm({ dictionary }: SignUpFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.message?.includes("email") || err.message?.includes("Email")) {
        setError(dictionary.emailInUseError);
      } else {
        setError(dictionary.signUpError);
      }
      console.error(err);
    } finally {
      setLoading(false);
  ...
  const handleGitHubSignUp = async () => {
    ...
    try {
      ...
      setError(dictionary.githubSignUpError);
      console.error(err);
      setLoading(false);
    }
  };
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      setError(dictionary.googleSignUpError);
      console.error(err);
      setLoading(false);
    }
  };
      ...
      >
        <div className="w-full max-w-md space-y-4">
          <div
            ...
            `}
          >
            <h2 className="text-3xl font-bold">{dictionary.pageTitle}</h2>
            ...
            <CardContent className="pt-2">
              <form className="space-y-4" onSubmit={handleSubmit}>
                ...
                <Button className="w-full" disabled={loading} type="submit">
                  {loading
                    ? dictionary.creatingAccountButton
