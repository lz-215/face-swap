...
export function UserTestimonialsSection() {
  ...
  return (
    ...
    >
      <h2 className="text-3xl md:text-4xl font-bold text-[#B6FF1C] mb-10 text-center">
      ...
      <div className="flex flex-col md:flex-row gap-8 justify-center items-stretch w-full max-w-6xl px-4">
        {testimonials.map((tItem, idx) => (
          <div
            key={idx}
            className="bg-[#232323] border border-[#333] rounded-lg shadow-lg p-7 flex-1 min-w-[300px] max-w-md flex flex-col"
          >
            ...
            <p className="text-base mb-6 text-[#e0e0e0]">{tItem.content}</p>
            <span className="font-semibold text-white mt-auto">- {tItem.author}</span>
          </div>
        ))}
      </div>
    </section>
